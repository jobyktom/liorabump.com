...
const schemaStatements = [
  ...
  "alter table profiles add column if not exists image varchar(500) null",
  "create table if not exists accounts (id varchar(191) primary key, user_id char(36) not null, type varchar(191) not null, provider varchar(191) not null, provider_account_id varchar(191) not null, refresh_token text, access_token text, expires_at int, token_type varchar(191), scope varchar(191), id_token text, session_state varchar(191), unique key accounts_provider_provider_account_id_unique (provider, provider_account_id), index accounts_user_id_idx (user_id), constraint accounts_user_id_fk foreign key (user_id) references profiles(id) on delete cascade) engine=InnoDB default charset=utf8mb4 collate=utf8mb4_unicode_ci",
  "create table if not exists sessions (id varchar(191) primary key, session_token varchar(191) not null unique, user_id char(36) not null, expires datetime not null, index sessions_user_id_idx (user_id), constraint sessions_user_id_fk foreign key (user_id) references profiles(id) on delete cascade) engine=InnoDB default charset=utf8mb4 collate=utf8mb4_unicode_ci",
  "create table if not exists verification_tokens (identifier varchar(191) not null, token varchar(191) not null unique, expires datetime not null, unique key verification_tokens_identifier_token_unique (identifier, token)) engine=InnoDB default charset=utf8mb4 collate=utf8mb4_unicode_ci",
